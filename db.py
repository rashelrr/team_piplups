...
def init_db():
    ...
    try:
        ...
        conn.execute("""CREATE TABLE IF NOT EXISTS REVIEWS(restaurant_name
                        TEXT NOT NULL, star ENUM('1', '2', '3', '4', '5')
                        NOT NULL, review TEXT NOT NULL,
                        UNI varchar(7) NOT NULL, PRIMARY
                        KEY(restaurant_name, UNI))""")
        conn.execute(
                     """CREATE TABLE IF NOT EXISTS UNI(UNI varchar(7) NOT NULL,
                     PRIMARY KEY(UNI))""")
...
#   Given a rating, compute and average rating for each restaurant and
#   return restaurant_name + star for the restaurants above that rating
def get_restaurants_above_ratings(rating):
    ...
    try:
        ...
        cur.execute("with avg_table as \
            (select restaurant_name, avg(star) as avg_star_rating from \
            REVIEWS group by restaurant_name) \
...
#   Given UNI and restaurant name, update the table with a new star
#   and a new review for that row
def edit_review(UNI, rest_name, new_star, new_review):
    conn = None
    try:
        conn = sqlite3.connect('Lion_Eats')
        cur = conn.cursor()
        cur.execute("update REVIEWS set star = ?, review = ? where UNI = ? \
                and restaurant_name = ?", new_star, new_review, UNI, rest_name)
        conn.commit()
    except Error as e:
        print(e)
...
#   Takes in a tuple (restaurant_name, star, review, UNI)
...
def add_review(row):
    ...
    try:
        conn = sqlite3.connect('Lion_Eats')
        cur = conn.cursor()
        cur.execute("INSERT INTO REVIEWS (restaurant_name, star, review, UNI) VALUES (?, ?, ?, ?)", row)
        conn.commit()
    except Error as e:
        print(e)
